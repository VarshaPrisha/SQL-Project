...
-- 3.availble Blood Stock for each blood group --------

WITH StockAvailble  AS (
    select
         Blood_Group, Sum(Quantity) as Availble_Stock
	from Blood_Stock
    Group by Blood_Group 
)

SELECT * FROM StockAvailble order by Availble_Stock desc;



--  4. Blood Group Required For Each hospitale --------

WITH BloodGroupInHospitale AS (
    select
         h.Hospital_Name, r.Blood_Group, count(r.Blood_Group) as No_of_people_required
	from Recipients r join Hospitals h ON r.Hospital_ID = h.Hospital_ID
    Group by r.Blood_Group, h.Hospital_Name
)

SELECT * FROM BloodGroupInHospitale order by No_of_people_required desc;


-- 5. TempTable For bloodGoup B Donor Info
create table #GroupBPositive
(
	Name varchar(100), 
...
Insert into #GroupBPositive(Name, Blood_Group, Phone) select Name, Blood_Group, Phone from Donors where Blood_Group = 'B+';

select * from #GroupBPositive;